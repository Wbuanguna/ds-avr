...
int analog_read(uint8_t pin) {
	...
	swiDelay(20000);
	send_chars(5, NULL_COMMAND, 0, 0, 0, 0);
	iprintf("0x%x 0x%x 0x%x 0x%x 0x%x \n", incoming[0], incoming[1], 
			incoming[2], incoming[3], incoming[4]);
	return incoming[1] | (incoming[2] << 8);
}
...
void spi_control(uint16_t m1, uint16_t m2) {
	AvrFifoMessage msg;
...
int main(void) {
	...
	while(1) {
		...
		if(keysDown() & KEY_A) {
			...
			ioport_set(0x2B, val);
			sysSetBusOwners(true, true);
		} else if(keysDown() & KEY_START && keysHeld() & KEY_SELECT) {
		    //profile_commands(10000);
		} else if(keysDown() & KEY_UP) {
			if (keysHeld() & KEY_L) m1 += 1000;
			else if(keysHeld() & KEY_A) m1 += 10;
			else m1 += 1;
			spi_control(m1, m2);
			iprintf("m1: %d\n", m1);
			...
			spi_control(m1, m2);
			iprintf("m1: %d\n", m1);
		} else if(keysDown() & KEY_RIGHT && m2 < 3) {
			m2 += 1;
			spi_control(m1, m2);
			iprintf("m2: %d\n", m2);
		} else if(keysDown() & KEY_LEFT && m2 > 0) {
			m2 -= 1;
			spi_control(m1, m2);
			iprintf("m2: %d\n", m2);
		} 
